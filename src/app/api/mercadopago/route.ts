  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Loja ou opção de plano inválida",
        },
        { status: 400 }
      )
    }
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Configuração inválida no catálogo de planos",
        },
        { status: 500 }
      )
    }
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Você não tem permissão para alterar esta loja",
        },
        { status: 403 }
      )
    }
    ...
    if (
      ...
      if (planoAtualError) {
        console.error(
          "Erro ao consultar o plano atual:",
          planoAtualError
        )
        ...
        return NextResponse.json(
          {
            error:
              "Erro ao calcular o crédito do plano atual",
          },
          { status: 500 }
        )
      }
      ...
      if (planoAtualData) {
        const planoAtual =
          planoAtualData as PlanoAtualCatalogo

        const valorPlanoAtual =
          Number(planoAtual.preco)
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "O crédito do seu plano atual cobre esta opção.",
          detalhes:
    ...
    if (pagamentoPendente) {
      return NextResponse.json(
        {
          error:
            "Já existe um pagamento recente aguardando conclusão para esta opção.",
          detalhes:
            "Aguarde alguns minutos ou conclua o pagamento que já foi aberto.",
        },
        { status: 409 }
      )
    }
    ...
    const response =
      await preference.create({
        body: {
          ...
          back_urls: {
            success:
              `${siteUrl}/lojista/loja/${lojaId}?pagamento=sucesso`,
            failure:
              `${siteUrl}/lojista/loja/${lojaId}?pagamento=falha`,
            pending:
              `${siteUrl}/lojista/loja/${lojaId}?pagamento=pendente`,
          },
    ...
    if (!response.id) {
      return NextResponse.json(
        {
          error:
            "O Mercado Pago não retornou a identificação da cobrança",
        },
        { status: 502 }
      )
    }
